var gh = (function() {
  ...
  function categorymodel(contentin){
    ...
    xhrWithAuth('POST', 'https://cors-anywhere.herokuapp.com/http://a089ec8b4801511eaaf7502ed822b4dd-589797411.us-west-2.elb.amazonaws.com/second-tier-api', false, categorize,data);  
  }

  function categorize(error, status, response) {
    if (!error && status == 200) {
      console.log("this msg belong to: ", response); 
      if(response==101){
        ...
        var data = {
          ...
          "topic2": JSON.parse(response)[1][0],
          "skip_body_notification":true
        };
      }
